...
- 규칙 3(raw 확인) 적용 누락. 편차 5(데이터 구조 주장 무검증)와 동일 계열 3번째.
- **결과 영향**: unknown 15.409% 의 일부를 설명한다. §19.2 본문은 수정하지 않는다(사전등록 봉인). CLI 가 결과를 보고 문자열을 고치지 않은 것은 규율 준수다.
...
- gap 구간별 success 비율은 관찰 5 로 이관 (편향이 아닌 신호 해석).
...
### 관찰 5 — prev=success 와 gap 의 상관 (편향 아님)
| gap | n | prev=success | error+unknown |
|---|---|---|---|
| < 20 | 653 | 390 (59.7%) | 244 (37.4%) |
| 20~99 | 387 | 344 (88.9%) | 35 (9.0%) |
| ≥ 100 | 232 | 221 (95.3%) | 10 (4.3%) |

- prev-tcid 는 창문을 보지 않고 prev Read 하나만 본다. 그럼에도 기울기가 남는다 → **창문 방식의 산물이 아니다.** 창문 방식(99%)은 이를 과장했다.
- unknown 196 중 171 (87.2%) 이 gap<20 에 위치.
- **가설 (미검증)**: 첫 Read 실패 → 즉시 재시도 → 짧은 gap. 첫 Read 성공 → 작업 지속 → 긴 gap 후 재읽기. 상관은 실측, 인과는 미검증. 규율 5.
- **함의**: v4 필터는 gap<20 에서 37% 를 걸러내고 gap≥100 에서 4% 만 걸러낸다. 필터가 목표한 패턴이 실재한다.
- **긴장**: 관찰 3'(gap 189 재읽기가 짧은 gap 과 같은 성격의 낭비인지 불명) 과 방향이 반대다. 긴 gap 후보는 첫읽기 성공률이 높아 깨끗하나, 낭비로서의 성격은 더 모호하다. 미해결.

### 관찰 6 — 인수인계 "무결한 발견 ②" 정정
- 원 서술: "'첫 시도 성공 여부'가 낭비 판정에 필수 — **42% 실측**"
- 41.66% (v3 761, 창문 any × `→`) 는 **두 버그의 합작**:
  (a) 정규식이 `\t` 포맷 9,941건(15.66%) 을 error 로 오분류
  (b) 창문 any 가 target 과 무관한 Read 를 봄
- v4'' (prev-tcid × `[→\t]`) 실측: error **93/1,272 = 7.31%**, unknown 196 = 15.41% (일부는 error 로 추정되나 미확정)
- **첫 Read 실패 비율 = 7.31% ~ 22.72%.** 42% 아님.
- **원칙은 유지**: 첫 시도 성공 여부는 판정에 필수다. 관찰 5 의 gap 상관이 이를 뒷받침한다. **크기만 붕괴했다.**
- 정직 경계: **"42%" 인용 금지.**
