...
| 버전 | 기존 후보 | 재검증 후보 | 기존 밀도 | 재검증 밀도 |
|---|---|---|---|---|
| v1 → v1' | 994 | **2,053** | 1.635% | **3.381%** |
| v2 → v2' | 817 | **1,272** | 1.344% | **2.095%** |
| v3 → v3' | 761 | **1,272** | 1.253% | **2.095%** |
| v4 → v4' | 424 | **858** | 0.698% | **1.413%** |

(v2' == v3' — turn_id 중복 dedup을 v1' 상류에서 처리하여 gap==0 후보가 원천 배제됨. `swechat_waste_cases_v2.csv`의 min gap = 2 확인.)

### 재실행 검증 항목

- **`unresolved_between == 0` ✓** (range-level: 0, file-level: 0) — SPEC §19.1 중단조건 통과
- **Edit tool_use `_path` 결측 == 0 ✓** — turn_type 필터로 완전 제거
- **"미확인 Edit" 조항 제거로 새로 승격된 후보: 1,114건** (기존 unresolved 1,115와 오차 1건 — 사실상 일치)
  * v1 유지: 939 / v1'에 없어짐: 55 (turn_id dedup 관련) / v1'에 신규: 1,114
