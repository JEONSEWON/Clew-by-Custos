...
## 18. 현재 단계 상세 — requery 오탐 진단 (게이트 설계용 데이터 수집)

**목적:** stage17에서 requery_known 성격 FIRE 4건 확인(진짜낭비 1: G8#1 동일
URL 재방문, 오탐 3: input 같으나 대상 URL 다름). 4건은 게이트 설계엔 얇다.
requery 성격 FIRE를 10건+ 모아 '오탐의 공통 원인'과 '진짜 낭비의 구별 신호'를
확실히 한 뒤에야 게이트를 설계한다. 이 단계는 진단이며 아무것도 고치지 않는다.
detect/·φ read-only.

**핵심 규율:**
- 판정은 사람이 원문으로. 도구 자동 판정 금지.
- 확증편향 경계: 가설('대상 다르면 오탐')에 안 맞는 케이스(대상 같은데 오탐,
  대상 다른데 진짜낭비)가 나오면 반드시 부각해서 기록. 가설 쪽으로만 해석 금지.
- '대상 동일성'을 어떻게 일반적으로 판정하는지가 핵심 미해결 질문. URL 파싱은
  TRAIL 오버핏 위험 — 일반적 신호를 데이터에서 찾는다.

### 범위
1. requery 성격 FIRE 수집: tool kind + input 정규화 동일인 FIRE를 집중 수집.
   기존 40개 + TRAIL 추가(총 60~80개)로 requery FIRE 10건+ 목표.
   (repeat_node 중 tool+동일input이 requery 성격. pingpong 제외.)
2. 각 requery FIRE에 대해 기록:
   - input_text (origin/candidate 정규화 동일 확인)
   - output_text 전문(대상 식별 가능하게 — URL/페이지/결과)
   - '대상 동일?' 사람 판정: 같은 대상 재조회 / 다른 대상 탐색 / 애매
   - '진짜 낭비?' 사람 판정: 낭비 / 정당한 탐색 / 애매
   - 부모 AGENT (게이트 통과했으니 동일할 것)
3. field_test/REQUERY_DIAGNOSIS.md 기록. 판정란 비워둠.
4. 집계: requery FIRE 총수, 진짜낭비 수, 오탐 수, '대상 동일성'과 '진짜 낭비'의
   상관(대상 동일↔낭비가 일치하는가). 확증편향 반례 별도 표기.

### 산출 목표 (사전등록)
- requery FIRE 10건+ 수집 + 사람 판정.
- '대상 동일성이 진짜 낭비를 가르는 신호인가'를 데이터로 답: 상관 강하면
  게이트 설계로(stage19), 약하거나 반례 많으면 다른 신호 재탐색.
- '대상 동일성을 일반적으로 판정하는 법'의 후보를 데이터에서 도출(URL 파싱
  아닌 것 우선).

**금지:** φ/detect 수정, 도구 자동 판정, 가설 확증편향, requery FIRE 10건 미만
으로 게이트 설계 강행, URL 문자열 파싱을 유일 해법으로 전제.
